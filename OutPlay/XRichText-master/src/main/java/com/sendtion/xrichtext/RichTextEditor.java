...
public class RichTextEditor extends ScrollView {
	...
	/**
	 * 根据绝对路径添加view，这里bitmap没有用，可以试试删去第一句有没有影响
	 *
	 * @param imagePath
	 */
	public void inserItmage(String url,String imagePath, int width) {
		Bitmap bmp = getScaledBitmap(imagePath, width);
		insertImage(url,bmp, imagePath);
	}

	/**
	 * 插入一张图片
	 */
	public void insertImage(String url,Bitmap bitmap, String imagePath) {
		...
		if (lastEditStr.length() == 0 || editStr1.length() == 0) {
			...
			addImageViewAtIndex(url,lastEditIndex, imagePath);
		} else {
			...
			addImageViewAtIndex(url,lastEditIndex + 1, imagePath);
	 ...
	 * 在特定位置添加ImageView
	 */
	public void addImageViewAtIndex(String url,final int index, String imagePath) {
